...
from utils.log import set_ptl_logger
    ...
    try:
        ...
        if "load_from_checkpoint" in config.model and config.model.load_from_checkpoint and not resume:
            try:
                ...
                model = load_model_checkpoint(model, opt.load_from_checkpoint)

            except Exception as ex:
                print(f"error load model ckpt: {ex}")
                raise ex
                # avoid size mismatch
        ...
        print(f"trainer logger: {trainer_kwargs['logger'] }")
        ...
        #    cb.wandb_logger = trainer_kwargs["logger"]
