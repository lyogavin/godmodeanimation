...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
                 ...
                 fps=10, log_to_tblogger=True, train_log_ratio=None, val_log_ratio=None, sample_prompts=None
                 ):
        super().__init__()
        ...
        self.train_log_ratio = train_log_ratio
        self.val_log_ratio = val_log_ratio
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            ...
            if isinstance(img, list) and isinstance(img[0], str):
                # a batch of captions
                filename = "string_{}_gs-{:06}_e-{:06}_b-{:06}_r-{:02}_{}.txt".format(
                    k,
                    global_step,
                    current_epoch,
                    batch_idx,
                ...
                with open(path, 'w') as f:
                    for i, txt in enumerate(img):
                        ...
                        to_log_wandb_str.append(f'idx={i}, txt={txt}\n')
                #wandb.log({filename: ''.join(to_log_wandb)}, step=global_step)
                to_log_wandb[f"{split}_prompttxt__{k}"] = ''.join(to_log_wandb_str)
                f.close()
            elif img.dim() == 5:
                # save video grids
                video = img # b,c,t,h,w
                ...
                filename = "video_{}_gs-{:06}_e-{:06}_b-{:06}_r-{:02}_{}.mp4".format(
                    k,
                    global_step,
                    current_epoch,
                    batch_idx,
                ...
                #wandb.log({path: wandb.Video(path)}, step=global_step)
                to_log_wandb[f"{split}_sample_video__{k}"] = wandb.Video(path, caption=filename)
                ...
                filename = "framesheet_{}_gs-{:06}_e-{:06}_b-{:06}_r-{:02}_{}.jpg".format(
                    k,
                    global_step,
                    current_epoch,
                    batch_idx,
                ...
                filename = "{}_gs-{:06}_e-{:06}_b-{:06}_r-{:02}_{}.jpg".format(
                    k,
                    global_step,
                    current_epoch,
                    batch_idx,
                ...
                #wandb.log({filename: wandb.Image(path)}, step=global_step)
                to_log_wandb[f"{split}_imggrid__{k}"] = wandb.Image(path, caption=filename)
                print('Finish!')

        wandb.log(to_log_wandb,
                  #step=global_step : try let lg to set step by itself to avoid unmatching
                  )
        return saved_videos
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train", rank=0):
        ...
        if (self.check_frequency(check_idx, split) and
            ...
            print("Log local ...")
            saved_videos = self.log_local(pl_module.logger.save_dir, split, images,
            ...
            if self.log_to_tblogger:
                ...
                #    pl_module.logger.log_video('example', v, pl_module.global_step)
                print('Finish!')
    ...
    def check_frequency(self, check_idx, split="train"):
        if split == 'val' or split == 'val_epoch_end_sample':
            return True
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=None):
        if not self.disabled and (pl_module.global_step > 0 or self.log_first_step):
            if self.train_log_ratio is None or random.random() < self.train_log_ratio:
                self.log_img(pl_module, batch, batch_idx, split="train", rank=trainer.global_rank)

    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=None):
        if not self.disabled and pl_module.global_step > 0 or self.log_first_step:
            if self.val_log_ratio is None or random.random() < self.val_log_ratio:
                self.log_img(pl_module, batch, batch_idx, split="val", rank=trainer.global_rank)
        if hasattr(pl_module, 'calibrate_grad_norm'):
            if (pl_module.calibrate_grad_norm and batch_idx % 25 == 0) and batch_idx > 0:
                self.log_gradients(trainer, pl_module, batch_idx=batch_idx)

        self.batch_image_shape = batch['image'].shape


    def on_validation_epoch_end(self, trainer, pl_module):
        print(f"on_validation_epoch_end, log sample gen images, self.batch_image_shape:{self.batch_image_shape}")
        if not self.disabled and self.batch_image_shape is not None and (pl_module.global_step > 0 or self.log_first_step):
            for prompt in self.sample_prompts:
                ...
                example["class_label"] = [""]
                example["class_name"] = [""]
                example["image"] = torch.zeros(self.batch_image_shape[1:]).unsqueeze(0)
                example["frame_stride"] = [1]

                to_print = {k:v if k!="image" else v.shape for k,v in example.items()}

                print(f"sample with mock batch: {to_print}")


                self.log_img(pl_module, example, 0, split="val_epoch_end_sample", rank=trainer.global_rank)
