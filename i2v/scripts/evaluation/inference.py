...
def get_parser():
    ...
    parser.add_argument("--v2v_image_path", type=str, default=None, help="v2v image path")
...
def run_inference(args, gpu_num, gpu_no, **kwargs):
    ...
    # load v2v image
    if args.v2v_image_path is not None:
        v2v_image = Image.open(args.v2v_image_path).convert('RGB')
        assert v2v_image.size == (512,320)
        v2v_image = torch.tensor(np.array(v2v_image)).permute(2, 0, 1).float() # [h,w,c] -> [c,h,w]

        v2v_image_t = 16
        # noise_shape = [batch_size, channels, frames, h, w]
        v2v_image = torch.cat([v2v_image.unsqueeze(1)  for i in range(v2v_image_t)], dim=1).unsqueeze(0)
        v2v_image = v2v_image.cuda()
        print(f"v2v_image shape: {v2v_image.shape}")
    ...
    for idx in range(0, n_rounds):
        ...
        batch_samples = batch_ddim_sampling(model, cond, noise_shape, args.n_samples, \
                                                args.ddim_steps, args.ddim_eta, args.unconditional_guidance_scale, v2v_image=v2v_image, **kwargs)
        ## b,samples,c,t,h,w
